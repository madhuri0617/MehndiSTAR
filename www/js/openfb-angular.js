...
angular.module('openfb', [])
    ...
    .factory('OpenFB', function ($rootScope, $q, $window, $http) {
        ...
        function login(fbScope) {
         ...
         var w = 300, h = 300;
         var   left = Number((window.screen.width/2)-((w/2+20))); 
         var tops = Number((window.screen.height/2)-((h/2+100)));

            loginWindow = window.open(FB_LOGIN_URL + '?client_id=' + fbAppId + '&redirect_uri=' + oauthRedirectURL + 
                '&response_type=token&display=popup&scope=' + fbScope, '_blank', 'location=no,clearcache=yes,width='+w+',height='+h+',top='+tops+',left='+left);
//            loginWindow = window.open(FB_LOGIN_URL + '?client_id=' + fbAppId + '&redirect_uri=' + oauthRedirectURL +
